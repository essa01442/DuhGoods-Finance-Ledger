          ...
          <div class="flex justify-between items-start gap-4">
            <div class="flex-1 min-w-0">
              ...
              <div class="grid grid-cols-2 gap-2 text-sm">
                <EvidenceCard
                  :record="match.leftRecord"
                  :label="t`السجل الأول`"
                />
                <EvidenceCard
                  :record="match.rightRecord"
                  :label="t`السجل الثاني`"
                />
              </div>
              ...
              <div class="mt-2 text-xs text-gray-500 dark:text-gray-400">
                <span
                  >{{ t`فارق المبلغ` }}:
                  <bdi dir="ltr">{{
                    formatMoney(match.amountDelta)
                  }}</bdi></span
                >
                <span class="mx-2">|</span>
                <span
                  >{{ t`فارق الأيام` }}:
                  <bdi dir="ltr">{{ match.dateDeltaDays }}</bdi></span
                >
              </div>
              ...
              <div v-if="match.reasonCodes" class="mt-1 text-xs">
                ...
                <span
                  v-for="code in parseReasonCodes(match.reasonCodes)"
                  :key="code"
                  class="
                    inline-block
                    mx-1
            ...
            <div
              ...
              class="text-green-600 text-sm shrink-0"
            >
              ✓ {{ t`مقبول` }}
            </div>
            <div
              v-else-if="match.status === 'rejected'"
              class="text-red-600 text-sm shrink-0"
            >
              ✗ {{ t`مرفوض` }}
            </div>
          </div>
          ...
          <div v-if="decisionNote[match.name] !== undefined" class="mt-2">
            ...
            <div class="flex gap-2 mt-1 justify-end">
              <Button
                ...
                :label="t`تأكيد الرفض`"
                type="danger"
                @click="confirmReject(match.name)"
              />
              <Button
                size="small"
                :label="t`إلغاء`"
                @click="cancelDecision(match.name)"
              />
            </div>
          </div>
        </div>
  ...
  setup(props) {
    const txTypeLabel = computed(() => {
      const map: Record<string, string> = {
        ...
        bank_credit: 'إيداع بنكي',
        bank_debit: 'سحب بنكي',
      };
      return (
        map[props.record?.transactionType as string] ??
        props.record?.transactionType
      );
    });
    return { txTypeLabel };
  },
...
export default defineComponent({
  ...
  setup() {
    ...
    const loadMatches = async () => {
      loading.value = true;
      try {
        const rows = await fyo.db.getAll(
        ...
        for (const row of rows) {
          const [left, right] = await Promise.all([
            fyo.db
              .get(ModelNameEnum.DuhGoodsImportRecord, row.leftRecord as string)
              .catch(() => ({})),
            fyo.db
              .get(
                ModelNameEnum.DuhGoodsImportRecord,
                row.rightRecord as string
              )
              .catch(() => ({})),
          ]);
          result.push({
    ...
    const parseReasonCodes = (codes: string): string[] => {
      try {
        ...
        return JSON.parse(codes) as string[];
      } catch {
        return [codes];
      }
    };
    ...
    const formatMoney = (val: unknown): string => {
      ...
      try {
        return fyo.pesa(String(val)).store;
      } catch {
        return String(val);
      }
    };
